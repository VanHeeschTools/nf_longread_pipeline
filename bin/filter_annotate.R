...
filterGTF <- function( novel_gtf_path, gtf_ref_path, tracking_file, min_occurrence, min_tpm, output_prefix, gtf_refseq_basename) {
  ...
  # v2.1 Add long-read evidence to reference
  lr_match_df <- novel_gtf_df %>%
  ...
  gtf_ref_df <- gtf_ref_df %>%
    left_join(lr_match_df, by = "transcript_id") %>%
    mutate(
      transcript_evidence = case_when(
        type != "transcript" ~ NA,
        is.na(transcript_evidence)       ~ "LR_no_evidence",
        transcript_evidence == "full_match"   ~ "LR_full_match",
        transcript_evidence == "partial_match"~ "LR_partial_match",
        TRUE ~ "LR_other_match"
      )
    )
  ...
  novel_gtf_df$transcript_evidence = "LR_novel"
